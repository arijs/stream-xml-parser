...
exports.getMatcherFrom = void 0;
var testlist_1 = require("./testlist");
var STRING = 'string';
var OBJECT = 'object';
exports.default = TreeMatcher;
function getMatcherFrom(item, elAdapter, opt) {
    if (item instanceof TreeMatcher)
        return item;
...
exports.getMatcherFrom = getMatcherFrom;
...
TreeMatcher.prototype = {
    ...
    getStringOpt: function (str) {
        ...
        if (mat) {
            ...
            switch (mat[2]) {
                case '?':
                    repeatMin = 0, repeatMax = 1;
                    break;
                case '*':
                    repeatMin = 0, repeatMax = Infinity;
                    break;
                case '+':
                    repeatMin = 1, repeatMax = Infinity;
                    break;
                default:
                    repeatMin = +mat[3] || 0;
                    repeatMax = mat[4]
    ...
    testRuleOrder: function (ruleList, itemList, testAdapter) {
        return testlist_1.default(ruleList, itemList, { testAdapter: testAdapter });
    },
    name: function (testName, opt) {
        var normalize = this.getNormalize(opt && opt.normalizeNodeName);
        if ('*' === testName)
            testName = this.acceptAll;
    ...
    testNodeName: function (node, method) {
        ...
        return this.testRuleItem(this.rulesName, nodeName, this.testNameRule, method);
    },
    attr: function (testAttr, opt) {
        ...
        var m = this.initRule(opt, def, function (attr) {
            return (self.testValue(testAttr.name, normName(attr.name)) &&
    ...
    getAttrTestFromString: function (item) {
        ...
        return { name: name, value: value, opt: opt };
    },
    ...
    testNodeAttrs: function (node, method) {
        return this.testRuleItemList(this.rulesAttrs, this.nodeAttrsToArray(node), this.testAttrRule, method || this.methodOrCount);
    },
    ...
    testPath: function (path, method) {
        return this.testRuleItem(this.rulesPath, path, this.testPathRule, method || this.methodOrList);
    },
    testAll: function (testNode, testPath) {
        ...
        return { success: success, name: name, attr: attr, path: path };
    }
};
