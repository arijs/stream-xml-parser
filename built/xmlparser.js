...
function XMLParser(opt) {
    ...
    this.getCustomParser = __assign({ attrValue: nop }, cp);
...
XMLParser.prototype = {
    ...
    decodeTag: function (ctag, ev) {
        ...
        return __assign(__assign({}, ctag), { name: name });
    },
    decodeAttr: function (cattr, ev) {
        ...
        return __assign(__assign({}, cattr), { name: name, value: value });
    },
    write: function (text, final) {
        ...
        function event(ev, id) {
            ...
            if (ctag)
                ev.tag = self.decodeTag(ctag, ev);
            if (cattr)
                ev.attr = self.decodeAttr(cattr, ev);
            if (buf)
                ev.text = self.decodeText(buf, ev);
            eventFn(ev);
        }
            ...
            switch (state) {
                ...
                case st_DECLARATION_START:
                    switch (c) {
                        ...
                        default:
                            ...
                            else if (buf.length < strCdata.length &&
                ...
                case st_TAG_NAMED_SELFCLOSE:
                    switch (c) {
                        case '>':
                            if (tagBeforeClose)
                                ctag.selfCloseSpace = tagBeforeClose;
                            if (buf)
                                ctag.endSpace = buf;
                            if (tagNameSlash)
                                eventTagNameSlash('155');
                            ctag.selfClose = true;
                            this.tagBeforeClose = tagBeforeClose = void 0;
                            eventEndTag('160');
                            break;
                ...
                case st_ATTR_VALUE_CUSTOM:
                    buf = ccp.attrValue.push(c, { attr: cattr, i: i, pos: pos, line: line, column: column });
                    if (buf != void 0) {
                        state = st_TAG_NAMED;
                        ccp.attrValue = null;
                        (cattr = buf.attr, i = buf.i, pos = buf.pos, line = buf.line, column = buf.column);
                        buf = '';
                        if (cattr)
                            eventTagAttr('290');
                    }
