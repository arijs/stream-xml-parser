...
HTMLTypeset.prototype = {
    ...
    scopePush: function (tag, style) {
        ...
        this.currentScope = __assign(__assign(__assign({}, scope), style), { tagOpen: tag, tagClose: null, parent: this.currentScope });
    ...
    treeTagOpen: function (tagName, tag) {
        var style = {};
        if (this.reBold.test(tagName))
            style.fontBold = true;
        if (this.reItalic.test(tagName))
    ...
    treeEvent: function (ev) {
        ...
        switch (evName) {
            case 'tagOpenEnd':
                this.treeTagOpen(tagName, tag);
                break;
            case 'tagCloseEnd':
                this.treeTagClose(tagName, tag);
                break;
            case 'text':
                this.treeText(ev.text);
                break;
        }
    },
    getResult: function () {
