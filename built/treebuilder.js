...
var events = {
    ...
    endInstruction: function (ev) {
        ...
        if (cs.tag !== TreeBuilder.TAG_INSTRUCTION) {
            var err = new TreeError('Current open tag is not an instruction', 120, this.getScopeState(ev, tag));
    ...
    endDeclaration: function (ev) {
        ...
        if (cs.tag !== TreeBuilder.TAG_DECLARATION) {
            var err = new TreeError('Current open tag is not a declaration', 121, this.getScopeState(ev, tag));
            this.errors.push(err);
            this.treeEvent('error', err, ev);
        }
        else {
    ...
    endComment: function (ev) {
        ...
        if (cs.tag !== TreeBuilder.TAG_COMMENT) {
            var err = new TreeError('Current open tag is not a comment', 122, this.getScopeState(ev, tag));
            this.errors.push(err);
            this.treeEvent('error', err, ev);
        }
        else {
...
function TreeError(message, code, state) {
    ...
    if (arguments.length > 2)
        this.extras = slice.call(arguments, 2);
}
...
function TreeBuilder(opt) {
    if (opt instanceof Function)
        opt = { event: opt };
    if (opt)
        opt = __assign(__assign({}, TreeBuilder.optDefault), opt);
    else
        opt = __assign({}, TreeBuilder.optDefault);
...
TreeBuilder.prototype = {
    ...
    getParserState: function (_a) {
        var parser = _a.parser, id = _a.id;
        return {
            chr: parser.c,
            pos: parser.pos,
            line: parser.line,
    ...
    resolveUnclosedTags: function (unclosed, ev) {
        ...
        while (uclen) {
            ...
            if (-1 === ucSelfPos) {
                err = new TreeError('Could not find child tag ' + JSON.stringify(ucTag.name) +
                    ' in parent tag ' + JSON.stringify(ucParent.name) +
                    ' list of ' + ucSiblings.length + ' children', 103);
                this.errors.push(err);
                this.treeEvent('error', err, ev);
                break;
            }
    ...
    findAndCloseTag: function (ev) {
        ...
        if (tagOpen) {
            ...
            if (streamTag.selfClose)
                breadcrumb.tag.selfClose = true;
            var unclosed = this.resolveUnclosedTags(tagOpen.unclosedTags, ev);
            var uclen = unclosed.length;
            if (uclen) {
                err = new TreeError((streamTag.selfClose ? 'Self closing tag ' : 'Close tag ') +
                    JSON.stringify(streamTag.name) + ' with ' + uclen + ' open tags', streamTag.selfClose ? 101 : 100);
                this.errors.push(err);
            ...
            err = new TreeError('Close tag ' + JSON.stringify(streamTag.name) + ' without opening tag', 102);
            this.errors.push(err);
