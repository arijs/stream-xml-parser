...
describe('TreeMatcher', () => {
	...
	describe('attribute matching', () => {
		...
		it('fails multiple rules for the same attribute without the allowMultipleRules option', () => {
			const { tree, elAdapter } = parse('<div class="many classes like bar and foo to showcase this"></div>');
			const tm = new TreeMatcher(elAdapter);
			tm.attr(['class', /\bfoo\b/]);
			tm.attr(['class', /\bbar\b/]);
			const result = tm.testNodeAttrs(tree[0]);
			assert.equal(result.success, false);
		});

		it('tests multiple rules for the same attribute with the allowMultipleRules option', () => {
			const { tree, elAdapter } = parse('<div class="many classes like bar and foo to showcase this"></div>');
			const tm = new TreeMatcher(elAdapter);
			tm.attr(['class', /\bfoo\b/, { allowMultipleRules: true }]);
			tm.attr(['class', /\bbar\b/]);
			const result = tm.testNodeAttrs(tree[0]);
			assert.equal(result.success, true);
		});

		it('tests multiple rules for the same attribute with the allowMultipleRules option in both rules', () => {
			const { tree, elAdapter } = parse('<div class="many classes like bar and foo to showcase this"></div>');
			const tm = new TreeMatcher(elAdapter);
			tm.attr(['class', /\bfoo\b/, { allowMultipleRules: true }]);
			tm.attr(['class', /\bbar\b/, { allowMultipleRules: true }]);
			const result = tm.testNodeAttrs(tree[0]);
			assert.equal(result.success, true);
		});
	});
